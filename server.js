...
http.listen(3000, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		...
		app.get("/user/id/:id", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					result.send({
						"status": "error",
						"message": "User does not exists through id"
					});
				} else {
					result.render("userProfile", {
						"user": user
		...
		app.post("/sendFriendRequest/:id", function (request, result) {
			var accessToken = request.fields.accessToken;
				...
				if (user == null) {
					...
					database.collection("users").findOne({
						...
						if (profile == null) {
							...
							var existingRequest = profile.friendRequests.find(function (request) {
								return request._id.toString() === user._id.toString() && request.status === "pending";
							});
							...
							if (existingRequest) {
								...
								// No existing request, proceed with sending the request
								database.collection("users").updateOne({
									"_id": profile._id
								}, {
									$push: {
		...
		app.get("/friendRequests", function (request, result){
			result.render('friendRequests');
		});
		
		app.post("/acceptFriendRequest/:id", function (request, result) {
			var accessToken = request.fields.accessToken;
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					database.collection("users").findOne({
						"_id": user._id,
						"friendRequests._id": ObjectId(requestId)
					}, function (error, userWithRequest) {
						if (userWithRequest == null) {
							...
							database.collection("users").updateOne({
								...
								var request = userWithRequest.friendRequests.find(function (request) {
